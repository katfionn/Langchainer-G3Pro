...
const AIConfigModal: React.FC<AIConfigModalProps> = ({ isOpen, onClose, onSave, language }) => {
  ...
  useEffect(() => {
    ...
    setActiveTab(baseConfigs[0]?.id || INTERNAL_ID);
  }, [isOpen]);
  ...
  const removeChannel = (id: string) => {
    ...
    if (activeTab === id) setActiveTab(configs[0]?.id || '');
  };
  ...
  const updateModel = (configId: string, modelId: string, field: keyof ModelInstance, value: any) => {
    setConfigs(configs.map(c => {
      if (c.id === configId) {
        let updatedModels = c.models.map(m => {
  ...
  const removeModel = (configId: string, modelId: string) => {
    ...
    if (config?.isInternal && config.models.length <= 1) return; 
    setConfigs(configs.map(c => c.id === configId ? { ...c, models: c.models.filter(m => m.id !== modelId) } : c));
  };
      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-5xl h-[85vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col">
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {currentConfig.models.map(model => (
                                <div key={model.id} className={`p-6 rounded-2xl border bg-slate-950/50 ${model.isPrimary ? 'border-indigo-500/50 ring-1 ring-indigo-500/20' : 'border-slate-800'}`}>
                                    <div className="flex flex-col md:flex-row gap-4">
                                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <input 
                                                ...
                                                className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white font-mono disabled:opacity-50"
                                            />
                                            {(currentConfig.channel === 'compatible' || currentConfig.channel === 'openrouter') && (
                                                <div className="md:col-span-2 space-y-1.5">
                                                    ...
                                                    <textarea 
                                                        ...
                                                        className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-xs font-mono text-emerald-400 outline-none"
                                                    />
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex md:flex-col justify-end gap-2 shrink-0">
                                            <button 
                                                ...
                                                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${model.isPrimary ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'bg-slate-800 text-slate-500'}`}
                                            >
                                                <Star className={`w-3 h-3 ${model.isPrimary ? 'fill-current' : ''}`} />
                                                {t.primary}
                                            </button>
                                            {!currentConfig.isInternal && (
                                                <button onClick={() => removeModel(currentConfig.id, model.id)} className="p-2 text-slate-600 hover:text-red-400">
        ...
        <div className="px-8 py-6 border-t border-slate-800 bg-slate-950/80 flex justify-between items-center">
            ...
            <div className="flex gap-3">
                ...
                <button onClick={handleSave} className="flex items-center gap-2 px-8 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold shadow-xl shadow-indigo-900/40">
